...
class Trainer:
    def __init__(
        ...
        self.pretrained_classifier = Net(
            state_shape=env.observation_space.shape[0],
            action_shape=env.action_space.shape[0],
            output_dim=algo.dim_c,
        ).to(algo.device)
        ...
        self.eval_metrics = EvalMetrics(
            len_discrete_code=algo.dim_c, device=algo.device
        )
    ...
    def train(self):
        ...
        state = self.env.reset(
            mode_idx=torch.argmax(self.algo.latent_c).detach().cpu().numpy()
        )
        for step in pbar:
            step = step + 1
            # Run a step
    ...
    def evaluate_ess_info_gail(self):
        ...
        mean_entropy, mean_nmi, avg_task_reward = self.eval_metrics.evaluate(
            self.pretrained_classifier, self.algo, self.env_test
        )
